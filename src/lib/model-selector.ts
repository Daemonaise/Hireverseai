...
// This function is synchronous and should NOT have 'use server'.
...
export function chooseModelBasedOnPrompt(promptContent: string): string {
  ...
  if (GOOGLE_API_KEY_RUNTIME)    availableModels.push(allModels.google);
  if (OPENAI_API_KEY_RUNTIME)    availableModels.push(...allModels.openai);
  if (ANTHROPIC_API_KEY_RUNTIME) availableModels.push(...allModels.anthropic);
